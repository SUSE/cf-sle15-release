### sle15 rootfs BOSH Release

This bosh release contains the rootfs package as well as a job that will
extract the package to `/var/vcap/packages/sle15/rootfs`.

#### Trusted certificates

Trusted certs can be installed in the rootfs by setting the
`sle15-rootfs.trusted_certs` property to the certificate chain in any
...
The following command will create and upload the sle15 BOSH release to the
BOSH director.
...
3. Run `bosh deployment ./manifests/bosh-lite/rootfs-smoke-test.yml && bosh deploy && bosh run errand sle15-smoke-test`
